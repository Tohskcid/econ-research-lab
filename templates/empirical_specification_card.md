...
- **核心處理變數 ($D_{it}$)**: `[政策虛擬變數 / 連續暴露度量 / 準實驗介入變數]`
- **核心參數 $\beta$ 意涵**: `[若 D 增加 1 單位，Y 變化之百分比或邊際效果]`

## 2. 固定效應與群聚推論 (Fixed Effects & Clustering)
- **個體固定效應 ($\alpha_i$)**: `[如: 個體/企業/縣市固定效應 吸納不隨時間變化的組間非觀測特徵]`
- **時間固定效應 ($\lambda_t$)**: `[如: 時間/年份固定效應 或 地區×年份固定效應 吸納總體宏觀衝擊]`
- **共變數矩陣 ($\mathbf{X}_{it}$)**: `[控制變數清單，注意避免放入壞控制變數 Bad Controls]`
- **標準誤群聚層級 (SE Clustering)**: `[如: Clustered at Treatment Assignment Level (縣/州/產業層級)]`
